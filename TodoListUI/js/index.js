        ...
        success: function(data){
            ...
            for (const index in todos){
                ...
                <div class="d-flex align-items-baseline mb-2 border p-2" onchange="updateTodo(${index})">
                    <input class="me-2 fw-bold" type="checkbox" ${item.isDone ? "checked" : ""} />
                    <span class="me-auto">${item.title}</span>
                    <button class="btn btn-sm btn-danger" onclick="deleteTodo(${item.id})"><i class="fa-solid fa-xmark"></i></button>
                </div>
                `);
            }
        }
